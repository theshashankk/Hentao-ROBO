...
import Mizuki.modules.sql.rules2_sql as sql
from Mizuki import *
...
async def _(event):
    ...
    if rules:
        await event.reply(
            "Click on the below button and check my pm to get this group's rules 😎🔥",
...
async def _(event):
    ...
    if len(args) == 2:
        ...
        await event.reply("Successfully set rules for this groups!")
...
async def _(event):
    ...
    await event.reply("Successfully cleared rules for this groups!")
